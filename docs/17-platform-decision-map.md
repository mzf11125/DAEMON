...
| Relationships | `Link`, junctions, optional Neo4j ([10-neo4j-graph-model.md](./10-neo4j-graph-model.md)) |
| Change history | Propagation → bronze; silver latest; gold rollups ([11](./11-data-platform-lakehouse.md)) |
| Search / object sets | `ScopedOntologySearch`, `GET /v1/search` |
| Ingest path | collect-sensing → gateway ingest pipeline |

**Data Ops breakdown:** Connect → Transform → Model → Analyze on the Data pillar — see [16-data-ops-lifecycle-map.md](./16-data-ops-lifecycle-map.md).
...
| Event-driven rules | [configs/governance/propagation.yaml](../configs/governance/propagation.yaml), `PropagationExecutor` |
| Workflows | `action-runtime`, `WorkflowOrchestrator` |
| NL / graph reasoning | `products/ontology-query` (LangGraph), competency questions ([09](./09-ontology-competency-questions.md)) |
| LLM orchestration | `products/customer-gpt`, optional OpenRouter |
| Governance gates | Breaking schema policies ([08](./08-semantic-governance-alignment.md)) |
...
| Action catalog | [configs/governance/action-catalog.yaml](../configs/governance/action-catalog.yaml) |
...
daemon-sdk: tenant/domain headers (`X-Daemon-Tenant`, `X-Daemon-Domain`), session RBAC, RLS on Postgres where enabled, audit journal. See [05-security-governance.md](./05-security-governance.md).
...
|--------------------|------------|
| Workshop / Slate low-code apps | [apps/dsdk-console](../apps/dsdk-console/) modules; no drag-and-drop widget composer |
...
- [18-enterprise-platform-map.md](./18-enterprise-platform-map.md) — Apollo/Foundry/AIP and `products/`
- [02-bounded-contexts.md](./02-bounded-contexts.md) — BC boundaries
- [08-semantic-governance-alignment.md](./08-semantic-governance-alignment.md) — governance alignment
